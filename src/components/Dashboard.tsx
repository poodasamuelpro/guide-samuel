...
export default function Dashboard({ state, onStartModule, onDownloadCertificate }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-[#fafaf8]">
      {/* ── HERO BANNER ─────────────────────────────────────── */}
      <div className="bg-[#1a2a4a] pt-20 pb-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <p className="text-[#f2994a] text-sm font-semibold mb-1">
                  Bon retour,
                </p>
              ...
              <div className="flex items-center gap-3">
                <div className="bg-white/10 rounded-2xl px-5 py-3 border border-white/10 text-center">
                  <p className="text-2xl font-extrabold text-[#f2994a]">{state.totalXP}</p>
                  <p className="text-[10px] text-white/50 uppercase tracking-widest mt-0.5">Points XP</p>
                </div>
                <div className="bg-white/10 rounded-2xl px-5 py-3 border border-white/10 text-center">
                  <p className="text-2xl font-extrabold text-white">{completedCount}<span className="text-white/40 text-lg">/5</span></p>
                  <p className="text-[10px] text-white/50 uppercase tracking-widest mt-0.5">Modules</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs font-bold text-[#f2994a]">{completedCount * 20}%</span>
              </div>
              <div className="h-2.5 bg-white/10 rounded-full overflow-hidden">
                <motion.div
                  className="h-full rounded-full"
                  style={{ background: 'linear-gradient(90deg, #f2994a, #e07b2a)' }}
                  initial={{ width: '0%' }}
        ...
        >
          ...
          <AnimatePresence>
            {allDone && (
              ...
              >
                <div className="bg-gradient-to-r from-[#1a2a4a] to-[#2d4570] p-5 flex flex-col sm:flex-row items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-[#f2994a] flex items-center justify-center shrink-0">
                ...
                >
                  <div
                    className={`card p-5 h-full flex flex-col transition-all duration-200 ${
                      unlocked && !done
                        ? 'card-interactive cursor-pointer border-2 border-transparent hover:border-[#f2994a]/40'
                        : done
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <h3 className="text-sm font-bold text-[#1a2a4a] leading-tight mt-0.5 line-clamp-2">
                          {mod.title}
                        </h3>
                      </div>
                    </div>
          ...
          {state.badges.length > 0 && (
            <motion.div variants={fadeUp} className="mb-8">
              <h2 className="text-lg font-extrabold text-[#1a2a4a] flex items-center gap-2 mb-4">
                <Award size={18} className="text-[#f2994a]" />
                Vos badges
              </h2>
                  ...
                  return (
                    <motion.div
                      ...
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-2xl border text-center ${
                        isChampion
                          ? 'bg-gradient-to-br from-[#1a2a4a] to-[#2d4570] border-[#f2994a]/40'
                          : 'bg-white border-[#f2994a]/30'
                      }`}
                    >
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center ${
                        isChampion ? 'bg-[#f2994a]' : 'bg-[#fef3e8]'
                      }`}>
                        <BadgeIcon size={16} className={isChampion ? 'text-white' : 'text-[#f2994a]'} />
                      </div>
                      <span className={`text-[10px] font-bold ${isChampion ? 'text-white' : 'text-[#1a2a4a]'}`}>
                        {label}
                      </span>
                    </motion.div>
                  );
          ...
          <motion.div variants={fadeUp}>
            <div className={`rounded-2xl p-5 border-2 flex flex-col sm:flex-row items-center gap-4 ${
              allDone
                ? 'bg-[#fef3e8] border-[#f2994a]/40'
                : 'bg-gray-50 border-dashed border-gray-200'
            }`}>
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${
                allDone ? 'bg-[#f2994a]' : 'bg-gray-200'
              }`}>
                <Download size={20} className={allDone ? 'text-white' : 'text-gray-400'} />
              </div>
              <div className="flex-1 text-center sm:text-left">
                <p className={`font-bold text-sm ${allDone ? 'text-[#1a2a4a]' : 'text-gray-400'}`}>
